...
export class BlockchainServer extends MessageServer<Message> {
  private readonly receiveMessagesAwaitingResponse = new Map<UUID, WebSocket>();
  ...
  private handleGetLongestChainRequest(
    ...
    if (this.clientIsNotAlone) {
      this.receiveMessagesAwaitingResponse.set(
  ...
  private handleGetLongestChainResponse(
    ...
    if (this.receiveMessagesAwaitingResponse.has(message.correlationId)) {
      const requestor = this.receiveMessagesAwaitingResponse.get(
        message.correlationId
      );
